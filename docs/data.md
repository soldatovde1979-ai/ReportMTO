...
#### ⚠️ `post` подменяется группой «Планирование ТО» — `postN` это НЕ ловит

Зафиксировано владельцем 17.09.2026 и проверено на рабочей выгрузке (22 020 строк).

**Суть.** `post` — родитель поста в иерархии 1С, и он **нестабилен**: посты переподчиняют.
Кто-то решил привязать часть постов к работам по ТО, создав группу `Планирование ТО`
и перенеся туда ремзоны. Именно поэтому в отчёте используется нормализованный `postN`,
а не `post` напрямую.

**Но обход не работает.** `fnNormalizeFields` строит `postN` из `post`, а не из `post_raw`:
...
...
WithPostN = Table.AddColumn(StatusBase, "postN",
    each PostNormalize(if HasPost then [post] else null), type text)
```

У перенесённых строк `post` = «Планирование ТО», слов «стк» и «прк» в нём нет —
нормализация возвращает строку как есть, и в отчёте появляется фантомная ремзона.

**Цифры на рабочей выгрузке:**

| Значение `postN` | Сейчас (из `post`) | Если строить из `post_raw` |
|---|---|---|
| СТК | 8 584 | **9 108** |
| ПРК | 2 440 | 2 440 |
| «Планирование ТО» | **524** | — (нет такой зоны) |

Все 524 строки «Планирования ТО» по `post_raw` оказываются `«10, СТК цех 1»`,
`«11, СТК цех 1»`, `«23, СТК цех 1»` — то есть это три поста СТК цеха 1.
**СТК недосчитывается 524 строк, и ровно на столько же существует фантомная зона.**

**Решение:** строить `postN` из `post_raw`. Но одной заменой поля не обойтись:
`post_raw` — сырое наименование, и `PostNormalize` на нём даёт мусор
(«Парковка ожидания ремонта Ангар №1», «1, Ангар №1», «Стоянка спец. техники Ангар №1»
вместо «Ангар №1»). Нужно дописать правило для «Ангар» и, возможно, для прочих зон.
Всего расхождений между двумя способами — 3 028 строк из 22 020.

#### `post_raw` — сырое наименование поста

Поле есть в выгрузке (заполнено у 63 % строк), в `docs/data.md` до 17.09.2026
не описано, в коде не используется. Примеры значений: «Парковка ожидания ремонта
СТК цех 1», «Парковка ожидания ремонта ПРК ДГМ», «Стоянка спец. техники СТК цех 1».

Из значений видно, что «пост» в выгрузке часто означает **парковку ожидания**,
а не ремонтный пост. Это стоит учитывать, читая блоки «по ремзонам».

#### Чего в выгрузке НЕТ (выяснено 17.09.2026)

> Проверено по полному списку полей `examples/`. Структура полей у выгрузок одна,
> поэтому вывод надёжен, в отличие от выводов о распределениях.

| Чего нет | Что есть вместо | Что из-за этого нельзя посчитать |
|---|---|---|
| **Дата текущего статуса документа** | `TekStatusPoDoc` без даты; `status_date` — дата **подписи**, заполнена не у всех строк | Отсчёт окна возврата «от последнего статуса». Доступно только приближение: максимальная `status_date` по наряду |
| **История смены статусов** | только текущий статус | Сколько наряд пробыл в «Ожидании ТМЦ»; разложение фазы «ремзона» на очередь и ремонт; честный помесячный остаток по статусам (сейчас он считает наряды, которые числятся в статусе **сейчас**) |
| **Наработка на дату наряда** | `odometer` / `engine_hours` — срез на момент выгрузки, и он меняется внутри машины вопреки контракту | «Отказов на 10 000 км»: нужна наработка ЗА ПЕРИОД |
| **Факт длительности ремонта** | `hourdlit` — план; `cost_Trudozatrat` — списанные часы | Разницу «план против факта по времени машины» (списанные часы ≠ время простоя) |

#### Поля, которые есть, но НЕ используются кодом

Проверено `grep` по `src\` 17.09.2026 — ни одного вхождения:

| Поле | Что это | Почему жаль |
|---|---|---|
| `Sektor` | сектор аэропорта: `Линейка Ш-1`, `Линейка Ш-2`, `Терминал B`, `Терминал C`, `Терминал D`; стабилен внутри машины | Готовый организационный разрез. Условия эксплуатации у линейки и терминала разные, и ни один блок отчёта этого не видит |
| `ts` | представление машины: номер + группа + модель, один к одному с `vehicle_number` | Читаемая подпись для поимённых списков машин: «5-26 Микроавтобус FORD TRANSIT» против «5-26» |

#### ⚠️ `Отказ` — это не поле выгрузки

Категория `Отказ`, по которой до 17.09.2026 отбирались наряды во всех блоках возвратов,
**в выгрузке отсутствует**. Это одна из категорий классификатора «характер работы»,
который строится в коде (`modContentZone.BuildRules` / `AddKind`) **по тексту
`defect_desc`** — списком ключевых слов. Рядом такие же вычисляемые категории:
«Оформление документов», «Обслуживание», «Дефектовка и осмотр», «Изготовление и замена узла».

Список ключевых слов писал разработчик, в документации выгрузки его нет, точность
проверить нечем. Отбор по нему зависит от того, как механик написал описание.

Замена — `zn_type` (поле выгрузки). В образце пять значений: `Внеплановый ремонт` (344
строки из 440), `Omnicomm` (56), `ТО3` (24), `ТО4` (8), `ТО1` (8).

#### ⚠️ Образец `examples/` устарел по содержимому

`examples/sppr_tablet_20260901_*.json` — выгрузка от 01.09.2026. **Структура полей
актуальна, содержимое — нет:** `odometer` и `engine_hours` в ней пусты во всех 440
строках, хотя в рабочей выгрузке заполнены (проверено владельцем по листу `tbDATA`).

Правило: по `examples/` проверяем **состав и типы полей**; доли, заполненность и
разбросы — только на рабочей выгрузке в `data\`.
...
класс `.period-cap`) — **с конкретными датами**, а не словами: «с начала года:
01.01.2026 → 21.09.2026», «8 недель: 2026-31 → 2026-38, по 21.09.2026». Подписи ставятся **централизованно** в `FillZonePlaceholders` и
...
| `{{REPORT_PERIOD}}` | обязательное поле «Период» в шапке: отчётная неделя, конец недели, границы «с начала года», месяц, дата обрыва выгрузки | `BuildPeriodLine(rw)` |
