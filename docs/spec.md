...
## 8а. Что где хранится: разбор снимков (выяснено 16–17.09.2026)

Контент-слой держит **два независимых снимка** одних и тех же строк `tbDATA`, и они
знают разное. Это не дублирование, а следствие разных единиц счёта, но при доработках
об это спотыкаешься первым делом.

| | `modContentZone` (`mZn`) | `modContentDisc` (`mOrd`) |
|---|---|---|
| Единица | заказ-наряд | заказ-наряд |
| Даты подписей (4 шт.) | **есть** (`Z_ACCG`…`Z_LEVD`) | есть (`E_TAG`…`E_TLD`) |
| **АРМ** подписей (ПК / ПЛАНШЕТ) | не было до 17.09.2026 (`Z_ALEVG`, `Z_ALEVD` добавлены) | **есть** (`E_AG`…`E_LD`) |
| Классификатор дефекта (`Z_KIND`, `Z_NODE`) | есть | нет |
| Пары возвратов | есть | нет |
| Свёртка нескольких записей статуса | **минимальная** дата | **последняя встреченная строка** |

Последняя строка — расхождение, а не решение: см. P1-6 аудита
[`docs/plans/audit_kod_i_slaidy_v1.0.md`](plans/audit_kod_i_slaidy_v1.0.md).
Интервал «приёмка → выбытие» на слайдах 2–3 и 7 считается от разных отметок.

### 8а.1 Признаки возврата

`RetPairs` строит пары «наряд → повторный наряд». Признаки на уровне наряда:

- `mRetOrd` — наряд **является** возвратом (второй в паре, тот, что приехал повторно);
- `mRetSrc` — **после** наряда случился возврат (первый в паре, чей ремонт не помог).
  Добавлен 17.09.2026: раньше признака первого наряда не существовало вовсе, хотя все
  счётчики велись именно по нему.

Оба словаря наполняются **одним** вызовом `RetPairs` (mode 2, окно 30) и не зависят
от фильтра периода — иначе сравнение двух лет невозможно: за прошлый год признак
не проставлялся.

### 8а.2 Как устроен подбор пар

- наряды машины по одному ключу дефекта сортируются по дате, и **каждый по очереди
  становится базой** — поэтому цепочка А → Б → В даёт два возврата, а наряд Б
  одновременно возврат для А и источник для В;
- в окне засчитывается **только ближайший** возврат (`Exit For`) — намеренно, иначе
  хроническая машина утраивает долю;
- отсчёт окна идёт от `zn_closed`, а если наряд не закрыт — **от даты создания**.
  Это подмена точки отсчёта: у незакрытого наряда «после ремонта» не наступило.
  Решение — в [`tz_vozvraty_planshet_v2.8.md`](plans/tz_vozvraty_planshet_v2.8.md) §12.

### 8а.3 Тихое отключение слоя ИИ

`GenerateReport` читает `AI/ENDPOINT` под `On Error Resume Next`, а `GetVariable`
при отсутствии ключа **бросает ошибку**. Одна общая проверка `Err.Number` на три шага
означает: нет ключа `AI/ENDPOINT` или `AI/MODEL` на листе `Variable` — запрос
не уходит, **даже если ключ API верный и промпт собрался**.

Второй источник того же симптома: `Environ$("AI_API_KEY")` читает окружение **процесса
Excel**. `setx` задаёт переменную для новых процессов — запущенный Excel её не видит,
нужен полный выход из всех `EXCEL.EXE`.

Диагностика: `DEBUG = 2`, затем `ReportMTO.log` рядом с книгой.
