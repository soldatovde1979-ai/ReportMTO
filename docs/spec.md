...
> промпт, палитра) — в **`specs/content-spec.md`**, который дополняет этот документ.
> Живой трекинг задач и найденных при реальном прогоне багов — в **`plans/next-steps.md`**
...
Для направления МТО роль Content Spec выполняет `specs/content-spec.md`; для нового направления пишется
...
```
MTO_Analytics\
├── ReportMTO.xlsm                    — локальная рабочая книга (вне git), собирается tools\build-report-mto.ps1
...
├── tools\
│   └── build-report-mto.ps1           — опциональная COM-автоматизация сборки .xlsm из src\ (см. §10.5)
├── examples\
│   └── reference-example.html         — визуальный референс (HTML-пример исходных требований), только сверка стиля
├── tmp_index.html                     — рабочий HTML-шаблон с плейсхолдерами {{...}} (см. §9.2)
├── data\                              — входящие JSON-выгрузки из 1С (sppr_tablet_<дата>_<время>.json), вне git
├── result\                            — готовые отчёты: Report_<YYYYMMDD_HHMMSS>.html (+ опционально .xlsx), вне git
├── install\                           — инструкции установки/обновления книги
├── docs\                              — документация: spec.md, data.md, logs.md, rules.md, index.md
│                                       — по темам: specs\ plans\ presentations\ archive\
└── tests\                             — тесты: код, данные, эталоны, инструкция запуска
```
...
| `AI/API_KEY` | `sk-xxxxxxxx` | **v7.1: вне книги.** Приоритет чтения: переменная окружения `AI_API_KEY` → файл `%APPDATA%\ReportMTO\deepseek.key` (UTF-8) → строка Variable (legacy, с предупреждением в лог) |
...
рантайм-логика их не перезаписывает.

> v7.1: `AI/API_KEY` хранится **вне книги**. Чтение — `modMain.ResolveAiApiKey()` с приоритетом
> переменная окружения `AI_API_KEY` → файл `%APPDATA%\ReportMTO\deepseek.key` (UTF-8) → строка
> `Variable` (legacy-режим для старых книг: каждый прогон пишет предупреждение в лог).
> Защита листа Excel не шифрует содержимое и снимается общедоступными средствами, поэтому
> строка `AI/API_KEY` в новых настройках не заполняется. Настройка — `install/install-v7.1.md`.
...
- **⚠️ История находки:** первая версия кода ошибочно меняла
  ...
  прогоне сборки в Excel; см. `plans/next-steps.md` за журналом.
...
4. **M-код запроса `Query-ImportJSON`** (структура шагов; конкретные поля — см. `specs/content-spec.md`):
...
```json
{
  ...
  "properties": {
    ...
    "arm": {
      ...
      "description": "Устройство, на котором сменился статус; «НЕ ПОДПИСАНО» — статус не зафиксирован ни на одном устройстве: такие строки в выгрузке 2026 идут с employee/status_date пустыми и в отчётность не входят. in_bounds=true при arm=«НЕ ПОДПИСАНО» означает: планшет приложен к метке (чек-ин), но документ не подписан — arm при этом корректен (подтверждено владельцем процесса 08.09.2026)"
    },
    ...
    "day_status": { "type": "integer", "description": "День даты статуса (разложение status_date). С v6 fnNormalizeFields вычисляет из status_date, если поля нет в выгрузке; пустой status_date -> null" },
    "month_status": { "type": "integer", "description": "Месяц даты статуса (разложение status_date). Вычисляется fnNormalizeFields (v6) при отсутствии в выгрузке; пустой status_date -> null" },
    "year_status": { "type": "integer", "description": "Год даты статуса (разложение status_date). Участвует в yearWeek и в «с начала года»; вычисляется fnNormalizeFields (v6) при отсутствии в выгрузке; пустой status_date -> null" },
    "week_status": { "type": "integer", "description": "ISO-неделя, в которую зафиксирован статус (Date.WeekOfYear с началом недели в понедельник); вычисляется fnNormalizeFields (v6) при отсутствии в выгрузке; пустой status_date -> null" },
    ...
    "in_bounds": {
      "type": "boolean",
      "description": "Флаг включения записи в отчётность (факт чек-ина — прикладывание планшета к метке). in_bounds=true при arm=«НЕ ПОДПИСАНО» означает, что планшет приложен к метке, но документ не подписан — это не ошибка выгрузки (подтверждено владельцем процесса 08.09.2026)"
    },
    "defekt_type": {
      "type": ["string", "null"],
...
> Причины отказа от Pivot — `specs/content-spec.md` §5. Скрытый лист `Pivots` больше не нужен.
...
**Системное сообщение (дословно; обновляется постановкой `docs/archive/task-for-coder.md` v1.1 — 7 слайдов):**
```
...
`defekt_type`, часы расхождения, число пар. Блок 6 уходит **только маркерами `[EMP_N]` без ФИО** и
агрегатами по людям (маркер, % планшет ПН/ПН-1, число подписаний). Поле `defect_desc` не включается
никогда (см. `specs/content-spec.md` §8).
...
```json
{
  "type": "object",
  "properties": {
    "slide1_conclusions": { "type": "string", "description": "До 4 предложений, вывод по обзору (Слайд 1)" },
    "slide2_conclusions": { "type": "string", "description": "До 4 предложений, вывод по ДЭНТ (Слайд 2)" },
    "slide3_conclusions": { "type": "string", "description": "До 4 предложений, вывод по ДГМ (Слайд 3)" },
    "slide4_conclusions": { "type": "string", "description": "До 4 предложений, вывод по синхронности дирекций (Слайд 4)" },
    "slide5_conclusions": { "type": "string", "description": "До 4 предложений, вывод по дефектам и рейтингу (Слайд 5)" },
    "slide6_conclusions": { "type": "string", "description": "До 4 предложений, вывод по динамике дирекций (Слайд 6)" },
    "slide7_conclusions": { "type": "string", "description": "До 4 предложений, вывод по динамике постов (Слайд 7)" }
  },
  "required": ["slide1_conclusions", "slide2_conclusions", "slide3_conclusions",
               "slide4_conclusions", "slide5_conclusions", "slide6_conclusions", "slide7_conclusions"]
}
```
...
3. ключи `slide1…slide7_conclusions` ищутся уже в развёрнутом тексте.
...
| Экранирование | `modHTMLEngine.HtmlEscape` — применяется направлением ко **всем** значениям, попадающим в HTML: полям источника (могут содержать `&`, `<`) и тексту внешнего ИИ (сторонний контент в файле, который открывают несколько человек). **Исключение:** JSON-дамп расшифровки `tbDATA` сериализуется через `JsonEscape` направления без `HtmlEscape` (кавычки-сущности ломают `JSON.parse`), плюс `«<»` → `\u003c` |
...
Конкретные блоки, слайды, промпт и палитра — см. `specs/content-spec.md`.
...
> количеству блоков/слайдов, не сверялось с заказчиком дословно (см. `plans/next-steps.md`).
>
> ⚠️ `{{BLOCK_4_GAUGE}}` содержит **раскрашенную матрицу «дирекция × неделя»**, а не круговой
> gauge: gauge из `specs/content-spec.md` §6 не реализован, вопрос о его необходимости открыт. Имя
...
5. `modContentMTO.ParseAIResponse` — двухшаговый разбор: `choices[0].message.content` → снятие
   JSON-экранирования → ключи `slide1…slide7_conclusions`, складываемые в module-level кэш;
   через `ByRef` наружу возвращаются прежние 3 значения (контракт §14).
6. `modContentMTO.BuildPlaceholders(slide3, slide4, slide5)` собирает словарь плейсхолдеров;
   ИИ-выводы при заполненном кэше читаются из него, при пустом — из параметров. Все значения
   проходят через `modHTMLEngine.HtmlEscape` (кроме JSON-дампа расшифровки — `JsonEscape`, §9.1).
...
Two пути (подробности — `tools/build-report-mto.ps1` и обсуждение в чате проекта):
...
- **Автоматизированно**: `tools/build-report-mto.ps1` — PowerShell-скрипт через COM-автоматизацию
...
> v1.1 постановки (`docs/archive/task-for-coder.md`): **сигнатура контракта не меняется**. Расширение с 3 до
> 7 ИИ-выводов (`slide1…slide7_conclusions`) реализуется внутри Content Spec через module-level кэш
> (аналог `mPairs`/`mPairsReady`): `ParseAIResponse` складывает все 7 ключей в кэш, а через
> `ByRef slide3/4/5` возвращает прежние 3 значения; `BuildPlaceholders` при заполненном кэше читает
> выводы из него, при пустом — fallback на входные параметры (нужно для `DebugGenerateOffline`,
> который вызывает `BuildPlaceholders(STUB, STUB, STUB)` без `ParseAIResponse`). Сброс кэша —
> в `BuildPivots`/`EnsureSnapshot` вместе с кэшами блоков.
...
> в Спринтах 1–2, здесь не перечисляются — их список в `specs/content-spec.md` §11.2 и в журнале
> изменений §20.
...
- ✅ **`AI/API_KEY` вне книги (закрыто кодом v7.1).** Защита листа Excel не шифрует содержимое
  и снимается общедоступными средствами, а книга передаётся между сотрудниками. Реализовано:
  `modMain.ResolveAiApiKey()` читает ключ из переменной окружения `AI_API_KEY` либо из файла
  `%APPDATA%\ReportMTO\deepseek.key` (UTF-8, права NTFS только на владельца); строка `Variable` —
  legacy-фоллбэк с предупреждением в лог. Регламент ИБ (кто и как выдаёт ключ) остаётся открытым.
- ⚠️ **Отчёт в `result\` содержит ФИО в открытом виде**, модель доступа — права на папку.
  Зафиксировано осознанно (§11, `specs/content-spec.md` §8); нужен регламент от ИБ — одним пакетом
  с вопросом про `AI/API_KEY`.
- ⚠️ **Построчный дамп `tbDATA` с ФИО за 2 недели встроен в автономный HTML-отчёт** (решение
  заказчика от 07.09.2026, `docs/presentations/Презентация и блоки данных.md` §1.1): копия файла отчёта теперь несёт
  построчные ПД (ФИО + точные даты/время), а не только агрегаты. Это расширение модели угроз
  относительно формулировки «доступ ограничен правами на папку `result\`»: NTFS-права действуют
  на оригинал, а копия файла контролю не подлежит. Согласовано с заказчиком; в отчёт попадает
  только то, что перечислено в §1.1 целевого документа, `defect_desc` — никогда.
...
- ⚠️ **Круговой gauge** (`specs/content-spec.md` §6) — не реализован; нужен ли он поверх раскрашенной матрицы.
...
- ⚠️ **Шаг 4 `tools/build-report-mto.ps1`** (Load To Table) не выполнялся ни разу.
...
- Исходное требование «отделить архитектуру и специфику для переиспользования под другие
  ...
  `specs/content-spec.md` (специфика МТО, v3.3.0). Ось разбиения — по переиспользуемости; между
...
- **Не отражено ни в §1–16, ни в остальных разделах:** структура `src/powerquery/` фактически шире
  описанной в §4 (7 файлов) — дерево файлов в §4 дополнено при стандартизации структуры (08.09.2026).
...
**07.09.2026 (синхронизация с постановкой v1.1)** — приведено в соответствие с
[`docs/archive/task-for-coder.md`](docs/archive/task-for-coder.md:1): контракт запроса/ответа ИИ расширен до 7
слайдов с псевдонимами `[EMP_N]` (§8.2, §8.3), оговорка `JsonEscape` для дампа расшифровки (§9.1),
примечание о неизменной сигнатуре контракта (§14), пункт ИБ о дампе расшифровки с ФИО (§17.2).
Правки кода — вне этого документа.

**08.09.2026 (v7.1)** — рестайлинг отчёта под дизайн-систему `examples/remzona-reports.html`
(шаблон `tmp_index.html` v2.0: системные шрифты Segoe UI/Consolas, офлайн, без Google Fonts;
KPI-сетка, mock-bar с заголовком периода и чипами-переключателями) и вынос `AI/API_KEY` за
пределы книги (§5.1, §17.2): `modMain.ResolveAiApiKey()` (env `AI_API_KEY` →
`%APPDATA%\ReportMTO\deepseek.key` → Variable legacy с предупреждением в лог).
