...
### `{{BLOCK_RETURN_ARM_RISK_YTD}}`, `{{BLOCK_RETURN_ARM_RISK_WK}}`

**Где:** слайд 7, сразу после `{{BLOCK_RETURN_KPI}}`, два блока в `.two-col`. **Что подставляет VBA:** `<div class="scroll"><table>` — риск повторного заезда по АРМ подписи «Готов к выбытию» (ПК / планшет / не определено): возвратов, из них повторных, % риска (`PctTd`) — + `calc-note` со сравнением ПК и планшета. YTD — с начала года (`InYtd` по дате создания наряда), WK — за отчётную неделю (по дате самой подписи выбытия).
...
**Где:** слайд 7, отдельная секция после блока риска по АРМ. **Что подставляет VBA:** `<div class="scroll"><table>` — топ-10 машин по числу возвратов («Готов к выбытию») за отчётную неделю: машина, группа техники, число возвратов, АРМ, направление, пост, статус по документу, дата выбытия + `calc-note`.
