...
- `[VBA/объявления]` `Dim` в середине процедуры (после исполняемых операторов) и имена переменных, совпадающие с ключевыми словами/функциями VBA, плодят повторные ошибки. -> Все `Dim` — в начале процедуры, до первого исполняемого оператора; имя переменной не должно совпадать с ключевым словом, функцией VBA или полем данных; после каждой правки прогоняй `tools/vba_lint_v1.0/vba_lint.py`.
