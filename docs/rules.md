...
## Новые файлы и дубликаты

- `[Проект/новый скрипт]` Прежде чем писать новую обработку или скрипт — поищи в проекте существующую с тем же назначением. -> Расширяй найденное; плодить два файла с одним функционалом запрещено.
- `[Проект/замена]` Новый скрипт заменил старый. -> Старый сразу переноси в архив (`tools\archive\`, документы — `docs\archive\`) и пометь в шапке, чем заменён.
...
- `[Git]` Push отклоняется при расхождении веток («fetch first»). -> Выполняй `git fetch` + `git merge origin/main --no-edit` (не используй rebase).
