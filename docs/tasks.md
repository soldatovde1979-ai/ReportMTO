...
> Версия 1.7 от 14.09.2026. Наведён порядок в docs\: исторические планы/ТЗ, манифест v6.1
...
- [x] 14.09.2026: наведение порядка в документации `docs\` — подробности в версии 1.7
      выше. `docs\plans\next-steps.md` оставлен на месте, его актуализация — в «Бэклоге».

- [x] 14.09.2026: постановка `docs\task-rep.md` преобразована в задачи —
      `docs\task-rep-v2.md` (весь текст разбит на задачи, контроль переноса в конце файла).
      Часть 1 перепроверена по коду: выполнено 22 из 24 задач. Расхождения:
      (1) Б4 «Без поста ремзоны» не выводится — `modContentZone` заполняет
      `BLOCK_NOPOST_WEEKLY`, а плейсхолдера в `tmp_index.html` нет;
      (2) Б3 «Какие дефекты были за неделю» (`BuildFlowDefekt`) и Б2 «Вид ремонта»
      (`BuildFlowZnType`) считают по всему массиву без фильтра отчётной недели.
      Код не менялся, исходник `task-rep.md` не тронут.
...
- [ ] Актуализировать `docs/plans/next-steps.md` (ревизия 24.08.2026). Открытые пункты,
      не покрытые другими записями этого файла: (1) формальное согласование замены
      ИИ-провайдера Gemini → DeepSeek; (2) подтверждение наличия >2 статусных записей
      на пару (number, direction) в данных 1С; (3) ротация файлов в `result\`;
      (4) перенос Core-части платформы на новое направление отчётности; (5) регламент
      доступа к `result\` (согласование с ИБ); (6) опционально — копия сводных таблиц
      в `Report_<дата>.xlsx`.
- [ ] Сделать свёртку БД
