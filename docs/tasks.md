...
- [x] 11.09.2026: `release.ps1` v1.1 — защита от отката. До установки читается
      `BUILD/VERSION` из книги: книга новее исходников -> `RELEASE_FAIL` (раньше старый
      код молча ложился поверх нового и версия в книге понижалась). Осознанный откат —
      флаг `-Force`, факт пишется в `CHANGELOG.md`. Равные версии с разным
      `BUILD/SRC_MD5` дают `BOOK_MD5_MISMATCH` (релиз с другой машины). Цена — одно
      открытие книги на чтение до установки. `docs\version-guide.md` v1.2: раздел
      «Работа с двух компьютеров» и правило pull -> release -> push.
