...
3. `docs\data.md` — описание данных: источники, форматы, поля, контракт промпта и шаблона
   (ревизия 16.09.2026, §3.3 — все 72 плейсхолдера). Парный `docs\spec.md` — спецификация системы.
...
5. `docs\archive\MTO_отчёт_v8.0_раскатка_v1.0.md` — состояние работ по отчёту v8.0 (выполнено, архив).
6. `docs\specs\brief-data-mto.md` — предметная область для внешнего аналитика.
7. `docs\tasks.md` — задачи: «Срочно» и «Бэклог».
8. `docs\version-guide.md` — версионирование сборки: одна команда раскатки, миграции, где смотреть версию.
9. `install\` — инструкции установки/обновления книги.
10. `docs\archive\` — только история: выполненные планы, старые инструкции установки.

> `docs\spec.md` создан 16.09.2026 (полная спецификация системы, собрана из кода `src\` v8.3);
> `docs\data.md` перенесён из `docs\specs\` и актуализирован. `docs\specs\content-spec.md`
> в репозитории отсутствует (удалён коммитом 7146c03 «docs cleanup»).
...
- **Читать можно:** README, `docs\index.md`, `docs\specs\`, `docs\plans\`, `docs\rules.md`,
  `docs\tasks.md`, `docs\version-guide.md`, `src\`, `tests\`, `install\`, `tools\`, `examples\`.
- **Без явного запроса не читать целиком:** `docs\archive\` (только конкретный файл по имени,
  если он назван в задаче), `data\` и `result\` (данные и готовые отчёты), книги `*.xlsm`
  (бинарные), Word-файлы в `docs\archive\` (первоисточники требований).
- **Не трогать без согласования:** книги, `data\`, `result\`, `bak\` и бэкапы `*.bak*`;
  перемещения и удаления файлов — только по одобренному плану; удалять можно только временные
  файлы, помеченные `_tmp_` / `_*`.
- **Гигиена папок:** не оставлять мусор в рабочих папках. Временные файлы — только в `tools\`
  с пометкой `_tmp_`, после использования удалять. Бэкапы — только в `bak\` (вне git), в `build\`
  и в корне бэкапов не оставлять. Лог книги — только `ReportMTO_log.txt` рядом с книгой,
  копии в `build\` не класть.
- **ТЗ:** именовать с префикса `tz_` и класть в `docs\plans\`; выполненные и устаревшие —
  в `docs\archive\`.
...
| `src\vba\modContentZone.bas` | Слайд 1 и слайды 5–8 («Техника»): 43 плейсхолдера, примитивы разметки и графики | **Content Spec** |
| `src\vba\modContentDisc.bas` | Слайды 2–4 («Дисциплина»): 16 плейсхолдеров | **Content Spec** |
...
| `tmp_index.html` | Рабочий HTML-шаблон v4.2: плейсхолдеры и навигация по 8 слайдам | **Content Spec** |
| `examples\reference-example.html` | Визуальный референс для сверки стиля (НЕ рабочий шаблон) | — |
| `docs\spec.md`, `docs\data.md` | Спецификация системы и данные: поля выгрузки, нормализация, контракт `tbDATA`, промпт, 72 плейсхолдера шаблона | — |
...
| `docs\archive\next-steps.md` | Ревизия 24.08.2026 (архив); актуальный трекинг задач — `docs\tasks.md` | — |
| `install\release.ps1` | **Точка входа раскатки**: установка + компиляция + версия + миграции + CHANGELOG | — |
| `install\VERSION`, `install\release.state` | Версия сборки (первая строка, ведёт install.ps1), ниже первой строки — человеческое описание изменений; состояние прошлого релиза (служебный) | — |
| `install\migrations\*.ps1` | Доп. код версии: то, что нужно доделать в книге сверх установки кода | — |
| `CHANGELOG.md` | Накопительный журнал версий: человеческое описание, затем техника; ведёт `install.ps1` | — |
