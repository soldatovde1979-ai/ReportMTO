...
.DESCRIPTION
    ...
    продолжит с того места, где остановился.
      ...
      1. Если $OutputPath (ReportMTO.xlsm) уже существует — открывает ЕГО (режим "продолжить").
         Если нет — открывает ReportMTO_starter.xlsx и сохраняет как $OutputPath (режим "с нуля").
...
.PARAMETER ProjectRoot
    Папка проекта (там, где лежат src\vba, src\powerquery, tmp_index.html, ReportMTO_starter.xlsx).
...
$starterXlsx = Join-Path $ProjectRoot "ReportMTO_starter.xlsx"
...
try {
    ...
    if (Test-Path $OutputPath) {
        ...
        if (-not (Test-Path $starterXlsx)) { throw "Не найден $starterXlsx — сначала положите заготовку в корень проекта." }
    ...
    # ⚠️ Классический .bas-импорт в VBA всегда читает файл в СИСТЕМНОЙ ANSI-кодировке
    ...
    $AnsiCodePage = 1251  # Windows-1251 (кириллица). Если система не русская — замените на
                          # нужный codepage (в Windows PowerShell 5.1 узнать: (Get-Culture).TextInfo.ANSICodePage)
    try {
    ...
    # 3.1 prmSourcePath — простая текстовая query-заглушка (НЕ именованный диапазон, см. чат/README)
    $prmFormula = '"' + ($SourcePathPlaceholder -replace '"', '""') + '"'
    Add-QueryIfMissing -Name "prmSourcePath" -Formula $prmFormula

    # 3.2 4 функции + Query-ImportJSON — в этом порядке, читаем M-код прямо из файлов
    $pqOrder = @("fnNormalizeFields", "fnComputeKey", "fnComputeGroupMetrics", "fnUpsert", "Query-ImportJSON")
