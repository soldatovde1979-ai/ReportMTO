...
try {
    ...
    Remove-Item (Join-Path $e2eDataDir "*") -Force -ErrorAction SilentlyContinue
    Copy-Item $json1 $e2eDataDir -Force
    $rows1 = RefreshSource $e2eDataDir
    $step1a = ($rows1 -eq 22)
    ...
    $rows2 = RefreshSource $e2eDataDir
    ...
    # We must use file path ($json2) to trigger IsFolder=false and run the Upsert logic!
    ...
    $actualResultFolder = Join-Path $tempDir "result"
    if (-not (Test-Path $actualResultFolder)) { New-Item -ItemType Directory -Path $actualResultFolder | Out-Null }
    
    # Set OUTPUT/RESULT_FOLDER in the workbook so Excel saves it to the temp directory
    $wb.Sheets.Item("Variable").ListObjects.Item(1).DataBodyRange.Cells(1,1).Value = "OUTPUT/RESULT_FOLDER"
    $null = $excel.Run("modMain.SetVariable", "OUTPUT/RESULT_FOLDER", $actualResultFolder)
    ...
    $latest = Get-ChildItem $actualResultFolder -Filter "debug_*.html" -ErrorAction SilentlyContinue |
        Where-Object { $_.LastWriteTime -ge $startTime.AddSeconds(-5) } |
        Sort-Object LastWriteTime -Descending | Select-Object -First 1
    if ($null -eq $latest) {
        StepResult "STEP4_OFFLINE" $false "no fresh debug_*.html in $actualResultFolder"
        ...
        Write-Output "  --- tail of VBA log (%TEMP%\ReportMTO.log) ---"
        $vbaLog = Join-Path $tempDir "ReportMTO.log"
        if (Test-Path $vbaLog) {
            $tail = Get-Content $vbaLog -Tail 20 -ErrorAction SilentlyContinue
            foreach ($ln in $tail) { Write-Output ("  " + $ln) }
        } else { Write-Output "  (no log)" }
    } else {
        # Template v4.0 has no drill-dump and no mto-report; the root marker of a
        # rendered report is the first slide container: <div class="slide on">.
        $t = [System.IO.File]::ReadAllText($latest.FullName)
        $ok4 = (-not $t.Contains("{{")) -and $t.Contains("slide on") -and $t.Contains("slide-nav") -and $t.Contains("[offline]")
