...
' --- Наборы фильтров (Content Spec §5: базовый фильтр in_bounds=ИСТИНА, arm<>"") ---
Private Function FBase() As Variant
    FBase = Array("in_bounds=True", "arm<>")
End Function

Private Function FArm() As Variant
    FArm = Array("in_bounds=True", "arm@=ПК;ПЛАНШЕТ")
End Function
...
' Матрица «строка × неделя» с долей планшета -> JSON-массив записей.
...
Private Function BuildBlock3KPI() As String
    ...
    BuildBlock3KPI = "<div class='kpi-grid'>" & _
        ...
        "<span class='kpi-delta'>Δ " & FormatDeltaPP(pctDGM_cur - pctDGM_prev) & "</span></div>" & _
        "<div class='kpi'><span class='kpi-cap'>ДЭНТ, % планшет (нед. " & Esc(label) & ")</span>" & _
        "<span class='kpi-val'>" & FormatPct(pctDENT_cur) & "</span>" & _
        "<span class='kpi-delta'>Δ " & FormatDeltaPP(pctDENT_cur - pctDENT_prev) & "</span></div>" & _
        "</div>"
End Function
...
Private Function Block6RankedTable(topN As Long, descending As Boolean) As String
    ...
    Set avgDelta = modAggregate.GroupAverage(Array("employee"), "deltaHours", _
        Array("in_bounds=True", "arm<>", "ready_for=Готов к выбытию"))
...
Private Function BuildSyncPairs() As Object
    ...
    For r = 1 To n
        If modAggregate.CellText(r, "ready_for") = "Готов к выбытию" Then
...
Private Function SyncAggregate(byPost As Boolean) As Object
    ...
    Dim srt As Variant
    srt = modAggregate.SortKeys(sums, Not byPost)   ' по неделям - численно, по неделя×пост - текстом
    Dim i As Long
...
' HTML-таблица Блока 7 (по неделям) или Блока 8 (по неделям × постам).
' Раньше в плейсхолдеры уходил компактный текст, предназначенный для промпта.
Private Function BuildSyncTable(byPost As Boolean) As String
    Dim agg As Object: Set agg = SyncAggregate(byPost)
    If agg.Count = 0 Then
        BuildSyncTable = "<p class='empty-note'>Нет пар «ДГМ ↔ ДЭНТ» по одному заказ-наряду " & _
            "для сопоставления.</p>"
        Exit Function
    End If
...
Private Function EmptyTable() As String
    EmptyTable = "<p class='empty-note'>Нет данных, удовлетворяющих фильтру " & _
        "(in_bounds = ИСТИНА, заполненный АРМ).</p>"
End Function
